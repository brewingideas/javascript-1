describe('ES6 numbers', function() {

    it('used to easily mistake numbers with leading zeros', function() {
        var octal = 071;
    ...
    it('should parse binary values with Number function', function() {
        var binNum = Number('0b1100');
        expect(binNum).toBe(12);
    });

    it('should expose parseInt and parseFloat', function() {
        expect(Number.parseInt(3)).toBe(3);
        expect(Number.parseFloat(3.52)).toBe(3.52);
    });

    it('should not convert strings when calling Nnmbers.isFinite vs global', function() {
        expect(isFinite('1')).toBe(true);
        expect(Number.isFinite('1')).toBe(false);
    });

    it('should not convert strings when calling Numbers.isNaN vs global', function() {
        expect(isNaN('NaN')).toBe(true);
        expect(Number.isNaN('NaN')).toBe(false);
    });

    it('should correctly determine integers with Number.isInteger', function() {
    ...
    it('should indicate safe integer with isSafeInteger', function() {
